...
class GitVersionManager:
    ...
    def __init__(
        self, version_manager: VersionManager, changelog_generator: ChangelogGenerator
    ):
    ...
    def create_release(
        self, bump_type: str = "patch", release_notes: Optional[str] = None
    ):
        """
...
def auto_version_from_commits():
    ...
    try:
        ...
        # Get recent commits
        commits = (
            subprocess.check_output(["git", "log", "--oneline", "-10"], text=True)
            .strip()
            .split("\n")
        )
        ...
        if release_info:
            logger.info(
                f"Auto-versioned: {bump_type} bump to {release_info['version']}"
            )
        else:
