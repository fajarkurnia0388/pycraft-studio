...
from ..utils.performance import build_tracker, performance_decorator
...
class EnhancedProjectBuilder(ProjectBuilder):
    ...
    def create_project_from_template(
        ...
        try:
            logger.info(
                f"Membuat proyek {project_name} dengan template {template_name}"
            )
            ...
            dependency_analysis = self.dependency_analyzer.analyze_project(
                str(project_path)
            )

            # Validasi struktur
            validation = self.build_validator.validate_project_structure(
                str(project_path)
            )
    ...
    def build_with_validation(
        ...
        try:
            ...
            validation_result = self.build_validator.validate_project_structure(
                project_path
            )
            ...
            dependency_validation = self.dependency_analyzer.validate_dependencies(
                project_path
            )
            ...
            # 4. Optimasi build
            optimized_args = self._optimize_build_args(
                additional_args, dependency_analysis
            )
    ...
    def analyze_project(self, project_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "build_readiness": self._assess_build_readiness(
                    validation, dependency_validation
                ),
            }
...
"""
    ...
    def _get_optimization_recommendations(
        self, dependency_analysis: Dict[str, Any], validation: Dict[str, Any]
    ) -> List[str]:
        ...
        if "requests" in external_deps:
            recommendations.append(
                "Pertimbangkan menggunakan --exclude-module untuk mengurangi ukuran"
            )

        if len(external_deps) > 10:
            recommendations.append(
                "Banyak dependencies - pertimbangkan virtual environment"
            )

        # Rekomendasi berdasarkan validasi
        if validation.get("score", 0) < 80:
            recommendations.append(
                "Tingkatkan struktur proyek untuk performa build yang lebih baik"
            )

        return recommendations

    def _calculate_overall_score(
        self, validation: Dict[str, Any], dependency_validation: Dict[str, Any]
    ) -> int:
...
"""
    ...
    def _get_next_steps(
        self, validation: Dict[str, Any], dependency_analysis: Dict[str, Any]
    ) -> str:
        ...
        if missing_deps:
            steps.append(
                f"2. Install dependencies yang hilang: {', '.join(missing_deps)}"
            )
