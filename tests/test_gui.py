...
class TestMainWindow:
    ...
    def test_start_build(self):
        ...
        with patch.object(self.window, "build_thread", create=True):
            with patch(
                "src.utils.file_utils.FileValidator.is_valid_python_file",
                return_value=True,
            ):
...
class TestEnhancedMainWindow:
    ...
    def test_open_project_wizard(self):
        ...
        with patch("tkinter.Toplevel") as mock_toplevel, patch(
            "src.gui.enhanced_main_window.ttk.Frame"
        ) as mock_frame, patch(
            "src.gui.enhanced_main_window.ttk.Label"
        ) as mock_label, patch(
            "src.gui.enhanced_main_window.ttk.Combobox"
        ) as mock_combo, patch(
            "tkinter.StringVar"
